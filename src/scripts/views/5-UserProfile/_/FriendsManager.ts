...
function FriendsManager() {
  ...
  if (
    $profile_friends.length > 0 &&
    $profile_friends[0].childElementCount > 0
  ) {
    ...
    let $showAllFriendsButtonContainer = $(
      "> div:nth-child(1)",
      $buttonsContainer,
    );
    ...
    let $removeSelectedButtonContainer = $(
      "> div:nth-child(4)",
      $buttonsContainer,
    );
    ...
    $showAllFriendsButton.on("click", () => {
      ...
      System.friends.forEach(friend => {
        ...
        friend.ranks.names.forEach(name => {
          let rankData = System.data.Brainly.defaultConfig.config.data.ranks.find(
            rank => {
