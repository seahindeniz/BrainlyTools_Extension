...
class ThemeColorChanger {
  ...
  $rainbow: JQuery<HTMLElement>;
  $colorValue: JQuery<HTMLElement>;
			...
			<article class="message is-info">
				...
				<div class="message-body">
					<div class="field is-horizontal">
						<div class="field-label has-text-centered">
							<label class="label">${
                System.data.locale.popup.extensionOptions.themeColor
                  .setYourColor
              }</label>
						</div>
                        ...
                        .rainbow
							...
							<div class="field">
								...
								<p class="control is-expanded">
									<input id="colorPicker" list="flatColors" class="input" type="color" placeholder="Text input" value="${
                    this.color
                  }">
								</p>
							</div>
							<div class="field">
								<p class="control">
									<input id="colorValue" list="flatColors" class="input" type="text" placeholder="${
                    System.data.locale.popup.extensionOptions.themeColor
                      .pickAColor
                  }"
                ...
                System.data.locale.popup.extensionOptions.themeColor
                  .colorFormatInfo
                    ...
                    System.data.locale.common.save
                  }</button>
								</div>
							</div>
						</div>
  ...
  BindHandlers() {
    this.$colorPicker.on(
      ...
      (
        event: JQuery.TriggeredEvent<
          ...
          HTMLInputElement
        >,
      ) => this.ColorInputHandler(event.target.value),
    );
    this.$colorValue.on(
      "input change",
      (
    ...
    this.$saveButton.on("click", this.SaveToStorage.bind(this));
    ...
    this.$rainbow.on("change", () => {
      ...
      this.$colorValue.val(colors).trigger("change"); // TODO check if it's working
    });
  }

  ColorInputHandler(color: string) {
  ...
  SaveToStorage() {
    const color = this.$colorValue.val();
