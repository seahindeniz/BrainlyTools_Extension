...
function IsDark(color: Color, limit?: number) {
...
class ThemeColorChanger {
  ...
  colors: string[];
  fontColor: string;
  fixedPrimaryColor: Color | string;
  head: HTMLElement;
  ...
  async Init() {
    ...
    this.head = await WaitForElement("head");
  ...
  PrepareMultiColor() {
    ...
    if (!IsDark(secondaryColor)) {
      secondaryColor = secondaryColor.lighten(0.3);
    } else {
      if (
        secondaryColor.hex().toLowerCase() !== DEFAULT_THEME_COLOR.toLowerCase()
      ) {
        this.fontColor = "#fff";
      }
    ...
    this.colors = [this.primaryColor, secondaryColor.hex().toLowerCase()];
  }
