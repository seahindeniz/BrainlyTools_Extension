let processAnnouncement = announcement => {
	...
	if (announcement.readed_by instanceof Array && announcement.readed_by.length > 0) {
		announcement.readed_by.forEach(reader => {
			...
			readers += `
			<a class="level-item is-inline-block" data-user-id="${reader.user_id.brainlyID}" title="${System.data.locale.texts.extension_options.announcements.readedOn.replace("%s", time)}" target="_blank">
